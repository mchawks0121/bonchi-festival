...
- `WaitingView` には、モード選択カード・接続状態ピル（プロジェクタークライアント時のみ）・**プレビュー URL カード（プロジェクターサーバー時のみ）**・「バグ狩り開始」ボタン・バグ一覧カード・ミッション説明カードが含まれます。
...
プロジェクターサーバーモードを選択すると、`PreviewServer`（TCP ポート **8765**）が自動起動し、同一 Wi-Fi 上の任意のブラウザからゲーム状態を確認できる HTML ステータスページを配信します。

### アクセス方法

1. `WaitingView` で「プロジェクター（サーバー）」を選択する。
2. 画面に表示される **プレビュー URL**（例: `http://192.168.x.x:8765`）をコピー、または「Safari で開く」ボタンをタップする。
3. Safari が新しいタブで URL を開き、ゲーム状態ページが表示される。
4. ページは 3 秒ごとに自動更新される。

### 表示内容

| 項目 | 内容 |
|------|------|
| ゲーム状態バッジ | 待機中 / プレイ中 / 終了 |
| タイム | 残り時間（秒） |
| スコア | 現在のスコア |
...
| クラス | 役割 |
|--------|------|
| `PreviewServer` | `Network.NWListener` ベースの TCP サーバー。HTML を生成して返す |
| `GameManager.previewURL` | `@Published URL?`。プロジェクターサーバーモード中のみ非 nil |
| `ProjectorPreviewURLCard` | `WaitingView` に表示されるカード UI。コピー・Safari 開くボタンを含む |

> **注意**: プレビューページはゲームのビジュアル（SpriteKit/SceneKit レンダリング）を表示するものではなく、ゲーム状態のテキスト情報を提供するステータスページです。
