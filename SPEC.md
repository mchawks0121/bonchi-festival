...
- スコアは各 iOS デバイスが独自に管理します（プロジェクターとの共有なし）。
...
└── World/
    ...
    ├── BugHunterScene.swift      … 2D バグゲームシーン（スコア計算なし）
    ├── BugSpawner.swift          … BugNode の生成・経路制御
    ├── NetProjectile.swift       … 飛んでくる網ノード（プレイヤー色対応）
...
- iOS 側 `GameManager` が `ARBugScene` または `BugHunterScene`（スタンドアロン時）の捕獲イベントを受けてスコアを加算します。
- プロジェクター側の `BugHunterScene` はスコアを保持せず、捕獲時に得点表示も行いません。
- プロジェクターから iOS へ送信する `GameStatePayload.score` は常に `0` です。
