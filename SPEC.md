...
2. **引っ張る** — 画面の **任意の方向にスワイプ** します。  
   引っ張る距離が強さ（power）になります（最大 300 pt）。  
   ARView 内に 3D の Y 字スリングショット（`SlingshotNode`）が常時表示され、  
...
| 🦠 | Virus (beetle) | 3 pt | 約 40 % | 普通 (70) | 55 pt | 自己増殖型ランタイムエラー。中程度の速度と得点。 |

※ stag (Glitch / toy_drummer.usdz) はパフォーマンス軽量化のためスポーン対象から除外しています。
...
│   ├── ARGameView.swift       … UIViewRepresentable。UIView コンテナに ARView (3D) + SKView (透過) を重ねる
│   │                             内部 Coordinator: ARSessionDelegate。ARAnchor → Bug3DNode.entity (Entity) + 不可視プロキシ SKNode
...
│   ├── Net3DNode.swift        … RealityKit Entity ラッパー。3D 飛翔網メッシュ（セグメントリム + 8 スポーク + 同心リング）
...
- 現行実装（As-Is）では、プロジェクターの初回自律スポーンは attach 直後の即時ではなく、`ProjectorBug3DCoordinator.startAutonomousSpawning()` の `scheduleNextAutonomousSpawn(after: 1.5)` により 1.5 秒後に発生します。
- クライアント接続時は `MCSessionState.connected` 直後に生存バグを再送しますが、その時点で生存バグが 0 体なら再送は行われず、次回スポーンまで待機になります。
- 他プロジェクトへ横展開する際の To-Be 必須要件:
  1. サーバーは `ProjectorBug3DCoordinator.attach(to:bugScene:)` で `startAutonomousSpawning()` 開始直後に初回バグを即時スポーンする（初回遅延禁止）
  2. 接続直後に同期対象が 0 体なら、同期送信前に 1 体を即時スポーンしてから送信する
  3. 同期送信直前に送信キュー（`bugSpawned` 件数）が 1 件以上であることを検証し、0 件なら送信処理を完了扱いにしない
  4. 即時スポーンが失敗した場合はサーバー側で 0.2 秒間隔・最大 10 回で再試行し、成功まで同期完了扱いにしない
  5. 10 回失敗した場合は `critical` ログを残し、対象クライアントを切断して再接続時に再同期フローを再実行する
  6. 補填スポーン（接続同期の穴埋めのために不足分を生成する処理）は必ずサーバー権威で実施し、クライアント側で補填しない
...
- `ARGameView.Coordinator` が `ARSessionDelegate` として、`SceneEvents.Update` サブスクリプション（メインスレッド）で毎フレーム投影を実行。
...
```
┌────────────────────────────────────────────────┐
│  ARView(cameraMode:.nonAR) — Bug3DNode.entity（3D バグ）常時アクティブ  │ ← 背面 (z=0)
...
- **ARView.environment.background**: `.color(UIColor(red: 0.05, green: 0.12, blue: 0.05, alpha: 1))`（暗い緑）
...
- `ProjectorBug3DCoordinator`（`WorldViewController.swift` 内に定義）: ARView(cameraMode:.nonAR) + PerspectiveCameraComponent (cameraZ=3.5、FOV=65°)。Bug3DNode.entity を `bugScaleMultiplier=10` でスケール拡大して表示。
...
| 最大ドラッグ距離 | **300 pt**（`maxDragDistance`）|
| 発射方向 | 任意方向（上下左右・斜め全方位） |

**操作手順**

1. 画面のスリングショットを任意の方向にドラッグする（最大 300 pt）。
...
power = min(dragLength / 300, 1.0)                     // 0.0〜1.0
```
...
| # | エフェクト | 詳細 |
|---|-----------|------|
| 1 | バグ消去 | `BugNode.captured()` + `Bug3DNode.captured()` による収縮後の点滅・非表示化 |
